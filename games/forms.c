...
// ---- actors -----------------------------------------------------------------
typedef struct { int32_t x, y, vx, vy; uint8_t form, grounded; } Actor;
...
static void init(void) {
    tables_init();
    g_act[0] = (Actor){ 300 << FP, FLOOR_Y << FP, 0, 0, F_UPRIGHT, 0 };
    g_act[1] = (Actor){ 340 << FP, FLOOR_Y << FP, 0, 0, F_UPRIGHT, 0 };
    g_checksum = 0;
...
static void tick(const Input in[2]) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        a->vx = in[i].x * (2 << FP);
        if (in[i].act && a->grounded) { a->vy = -(5 << FP); a->grounded = 0; g_events |= EV_JUMP; }
        a->vy += (1 << FP) / 4;
