...
int main(int argc, char **argv) {
    ...
    for (int a = 1; a < argc; a++) {
        ...
        // The data directory rides along with --version because "where did it put my file" is a
        // question a user asks over a chat window, and cvx_data_dir() is the only honest answer —
        // it is resolved, not assumed, so a translocated .app reports the fallback it actually uses.
        else if (!strcmp(argv[a], "--version")) { printf("cvertex %s\ndata: %s\n", CVERTEX_VERSION, cvx_data_dir()); return 0; }
